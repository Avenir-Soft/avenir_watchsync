// Watch Sync — ловит <video> (в т.ч. Shadow DOM/iframe), шлёт позицию, перематывает,
...
// стабильный ключ серии из kodik-URL (без протокола и качества)
function epKey(u) {
...
function tryResume(v) {
  ...
  send({ type: "getResume" }, (res) => {
    ...
    // сериал: сохранена конкретная серия, а это другая — не перематываем (ждём переключения серии)
...
// ── только верхний фрейм: постер + переключение на нужную серию ──
if (window === window.top) {
  ...
  // сериал: если сохранена другая серия — находим её в списке и кликаем
  ...
  const trySwitchEpisode = () => {
    ...
    send({ type: "getResume" }, (res) => {
      ...
      if (!key) { epSwitched = true; return; } // не сериал / нет kodik-серии
